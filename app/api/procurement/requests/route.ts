...
export async function GET() {
 try {
  ...
  const { data, error } = await admin
   ...
   .select('*, lines:ProcurementRequestLine(*, item:Ingredient(name, unit))')
...
export async function POST(request: NextRequest) {
 try {
  ...
  await logActivityServer({
   ...
   metadata: { item: itemName, qty: roundedQty, unit },
   userId: auth.userId,
   userRole: auth.role,
  ...
  console.error('Procurement request create error:', error);
  return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
 }
}
