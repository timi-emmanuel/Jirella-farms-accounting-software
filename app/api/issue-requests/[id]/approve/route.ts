...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
 try {
  ...
  await logActivityServer({
   action: 'ISSUE_REQUEST_APPROVED',
   entityType: 'IssueRequest',
   entityId: id,
   description: `Approved issue request for ${requestRow.quantity}${requestRow.unit} of ${requestRow.itemName}`,
  ...
  console.error('Issue request approve error:', error);
