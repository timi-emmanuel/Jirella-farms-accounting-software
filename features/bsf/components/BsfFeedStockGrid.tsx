...
export function BsfFeedStockGrid() {
  const [rowData, setRowData] = useState<StockRow[]>([]);
  const [loading, setLoading] = useState(true);

  const loadData = async () => {
    setLoading(true);
    const response = await fetch('/api/finished-goods/location?code=BSF&module=FEED_MILL');
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      console.error('BSF feed stock load error:', payload.error || response.statusText);
    } else {
