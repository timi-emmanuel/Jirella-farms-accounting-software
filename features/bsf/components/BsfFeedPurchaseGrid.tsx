...
export function BsfFeedPurchaseGrid() {
  const [rowData, setRowData] = useState<PurchaseRow[]>([]);
  const [loading, setLoading] = useState(true);

  const loadData = async () => {
    setLoading(true);
    const response = await fetch('/api/finished-goods/internal-sales?targetModule=BSF');
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      console.error('BSF feed purchases load error:', payload.error || response.statusText);
    } else {
