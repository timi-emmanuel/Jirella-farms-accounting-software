...
export function CatfishExpenseGrid() {
  ...
  const loadData = async () => {
    setLoading(true);
    const response = await fetch("/api/expenses?module=CATFISH");
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      console.error("Catfish expense load error:", payload.error || response.statusText);
    } else {
            ...
            <form onSubmit={handleAdd} className="space-y-5 py-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Labor, utilities, treatment, maintenance"
                  required
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="expenseAmount">Amount (N)</Label>
                <Input
                  id="expenseAmount"
                  type="number"
                  step="0.01"
