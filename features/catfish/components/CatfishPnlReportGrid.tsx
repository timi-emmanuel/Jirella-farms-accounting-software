...
export function CatfishPnlReportGrid() {
  const [rowData, setRowData] = useState<PnlRow[]>([]);
  const [loading, setLoading] = useState(true);

  const loadData = async () => {
    setLoading(true);
    const response = await fetch('/api/catfish/reports/pnl');
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      console.error('Catfish P&L load error:', payload.error || response.statusText);
    } else {
